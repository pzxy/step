...
1 Compare
他是比较[]byte中的每个字节,相等的话继续比较.不等的话,直接返回
按字典顺序返回一个比较两个字节片段的整数。
...
3 ContainsAny 报告字符中的任何 UTF-8 编码的 Unicode 代码点是否在 d 中
看字节中utf8字符串是否包含 字符串   忽视空格
...
4 ContainsRune 报告 Unicode 代码点 r 是否在 b 之内。
看字节数组是否包含 单个字符
...
5 Count 计算s中不重叠实例的数量。如果 sep 为空片段，则 Count 返回1 + s中的 Unicode 代码点数。
计算目的字符个数,当为空时返回长度+1
func countDemo() {
...
7 EqualFold 报告无论 s 和 t，解释为 UTF-8 字符串，在 Unicode 大小写折叠下是否相等。
EqualFold 比较相等忽视大小写,但是不忽视空格,会比较空格
func equalFoldDemo() {
...
9 FieldsFunc按照指定字符分割字符串 func FieldsFunc(s []byte, f func(rune) bool) [][]byte
...
func repeatDemo() {
	fmt.Printf("ba%s", bytes.Repeat([]byte("na"), 2))//bnana
}
